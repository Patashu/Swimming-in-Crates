...
func change_pressed(actor: Actor, new_value: bool, pos: Vector2, chrono: int) -> void:
	...
	for wire in wires:
		if wire[0] == button_id and int(wire[1]) == wire[1] and hatches.size() > wire[1] and -hatches.size() <= wire[1]:
...
func looks_like_level(custom: String) -> bool:
	...
	if custom.find("SwimmingInCratesPuzzleStart") >= 0 and custom.find("SwimmingInCratesPuzzleEnd") >= 0:
		return true;
	return false;
